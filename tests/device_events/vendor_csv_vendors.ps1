...
if ($requireAuth) {
  ...
  if (-not $adminKey -and -not $opKey) {
    Write-Host 'SKIP: vendor_csv_vendors (TEST_API_KEY_* missing under REQUIRE_AUTH)'
    exit 0
  }
...
try {
  ...
  try {
    ...
    if ($status -eq 404 -or $errorValue -eq 'not_found' -or $textLower.Contains('allow_test_endpoints')) {
      Write-Host 'SKIP: vendor_csv_vendors (test endpoints disabled)'
      exit 0
    }
    throw
  }
