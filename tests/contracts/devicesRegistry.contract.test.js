...
async function run() {
  ...
  assert.ok(Array.isArray(listRes.body.value), 'devices list should include value array');
  assert.strictEqual(typeof listRes.body.Count, 'number', 'devices list should include Count number');
  ...
  const putRes = await requestJson({
    ...
    body: {
      provider: 'zkteco',
      device_name: `Device Contract ${runId}`,
      active: true,
  ...
  assert.strictEqual(getRes.body.device_uid, deviceUid, 'device GET should return matching device_uid');

  console.log('devices registry contract tests passed');
}
