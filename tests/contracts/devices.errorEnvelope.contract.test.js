...
async function run() {
  ...
  assert.strictEqual(missingRes.body.code, 'LOOKUP_NOT_FOUND', 'missing device code mismatch');
  assert.ok(
    missingRes.body.details && missingRes.body.details.kind === 'lookup_error',
    'missing device details.kind should be lookup_error'
  );
  assert.strictEqual(
    missingRes.body.details.error,
    'device_not_found',
    'missing device details.error mismatch'
  );
